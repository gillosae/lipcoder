...
export async function findImageFiles(): Promise<string[]> {
    ...
    for (const folder of workspaceFolders) {
        const files = await vscode.workspace.findFiles(
            new vscode.RelativePattern(folder, '**/*.{jpg,jpeg,png,gif,webp,bmp}'),
            '**/node_modules/**',
...
/**
 * Analyze currently open image file or show selection dialog
 */
export async function selectAndAnalyzeImage(): Promise<void> {
    try {
        ...
        // No image file is currently open, show selection dialog
