...
# macOS Native TTS Alphabet Generation Script
# Generates alphabet PCM files using macOS native 'say' command

# Directory to save PCM files
OUT_DIR="alphabet_macos"
mkdir -p "$OUT_DIR"
...
echo -e "${BLUE}🍎 Generating alphabet PCM files using macOS native TTS...${NC}"
...
# Declare arrays: letter => pronunciation
# For macOS voice, use the letter itself instead of phonetic pronunciation
LETTERS=(a b c d e f g h i j k l m n o p q r s t u v w x y z)
PRONUNCIATIONS=(a b c d e f g h i j k l m n o p q r s t u v w x y z)

echo -e "${YELLOW}📝 Generating ${#LETTERS[@]} alphabet sounds...${NC}"

# Generate each letter as PCM
for i in "${!LETTERS[@]}"; do
  letter="${LETTERS[$i]}"
  ...
  echo -e "${BLUE}🔊 Generating: ${letter} → \"${text}\" → ${final_pcm}${NC}"
  ...
  if [ $? -eq 0 ] && [ -f "$temp_wav" ]; then
    ...
    echo -e "${RED}❌ Failed to generate TTS for: ${letter}${NC}"
  fi
done

echo -e "${GREEN}🎉 All alphabet PCM files generated using macOS TTS in ${OUT_DIR}${NC}"
